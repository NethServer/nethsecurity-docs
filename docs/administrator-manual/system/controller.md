...
Each controller user can enable Two Factor Authentication (2FA) to increase the security of the account. To enable 2FA, follow the same steps documented inside the firewall web interface: [NethSecurity UI 2FA](../users-objects/administrative_users.md#2fa-section).
...
Please be aware that units might not send updated information when undergoing upgrading process prior to unit version 1.3.0. To refresh manually the information use the **Sync unit info** button in the unit menu.
...
3.  Report issues: if you encounter any unusual behavior or errors, especially after seeing a version warning, follow the [troubleshooting](../../tutorial/troubleshooting.md) procedure.
