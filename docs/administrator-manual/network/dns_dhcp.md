...
For privacy-focused DNS configuration using encrypted connections, see [DNS over HTTPS with filtering](../../tutorial/dns-over-https.md) for DNS over HTTPS (DoH) setup.
...
- Replace `\<LOCAL_ADDR\>` with the firewall's IP address to listen for DHCP requests from clients.
