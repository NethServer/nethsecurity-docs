...
Users inside the local database can be granted administrative privileges on the web user interface by enabling the `Administrator user` option; the user must have a password set. See [Administrative users](./administrative_users.md) for details.
...
If you are connecting to a Windows Server 2025 (or newer) Active Directory, the LDAP bind may fail due to stricter security defaults. See [Active Directory LDAP bind issues](../../tutorial/active-directory-ldap-bind-issues.md) for the fix and recommended NethSecurity configuration.
